...
body_mass_bar <- penguins_df %>%
  group_by(species) %>%
  summarize(
    mean_body_mass = mean(body_mass_g)
  ) %>%
  ggplot() +
    geom_bar(aes(x = mean_body_mass, y = species), stat = "identity")
...
```{r}
# use geom_errorbar (specify y, xmin, and xmax)
body_mass_bar + 
...
```{r}
body_mass_violin <- penguins_df %>%
  ggplot() + 
  geom_violin(aes(y = species, x = body_mass_g, color = species))
